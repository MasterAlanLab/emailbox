...
落地成一整页而不是 `Dialog`：左边一块 16 行的粘贴区，右边 320px 放格式、目标分组、
冲突策略三个选择。提交后在页面底部给逐行统计（总数 / 新建 / 更新 / 跳过 / 失败）
与失败明细，明细超过 200 条截断。
...
示例表下面三句提示解决的都是真实的失败路径：非 Outlook 服务商要填邮箱设置里生成的
授权码而不是登录密码（否则报「授权码错误，或未在邮箱设置中开启 IMAP 服务」）；
Outlook 只能走 4 段 OAuth；域名不在表里就用自定义 IMAP 的 4 段写法。
