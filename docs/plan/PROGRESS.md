...
- [x] 邮箱侧栏自动收起后保留「管理分组」入口：桌面收起态展开侧栏并打开分组段，
      1280px 以下在分组筛选旁打开同一套管理面板；编辑、排序、删除和新建继续复用原有接口与校验。
- [x] 对齐邮箱页布局文档：`9ab6215` 已移除纵向 `SplitPane`，原 06 文档与 `AGENTS.md`
      仍描述右栏上下切分；现按实际的账号、邮件、正文逐级并列与断点隐藏规则记录。
- [x] 修正管理面板内的分组菜单层级：Kumo `DropdownMenu.Content` 的 `className`
      只落在内层 `MenuPopup`，单改 `z-index` 不会抬高外层 `MenuPositioner`；菜单改为
      挂到管理面板根节点，并把 `zIndex` 直接传给 positioner，编辑项不再被面板遮挡。
...
- [x] 邮箱工作台增加可记忆的侧栏收起：选中邮箱后自动缩成图标列，收起后仍保留状态与分组筛选
- [x] 账号列表、邮件列表、邮件正文按选择状态逐级展开，当前内容面板占据剩余空间
- [x] 邮件正文只在选中邮件后展开；未选中时收件箱独占高度，避免空预览区挤压列表
- [x] 普通登录后页面统一放宽 `PageShell` 外壳，宽屏下减少无意义的两侧留白
...
### 删除资源推荐页（2026-09-07，用户要求）

`/resources` 整页删掉，左侧次级导航的「资源」入口一并去掉，README 末尾的推广清单也删了。
项目里不再放 affiliate 内容——这些链接和邮箱托管本身没有关系，留在导航里等于让每个用户
每天都路过一次广告位。`ResourcesPage.tsx` 是唯一的载体，删文件 + 摘路由即可，没有后端
残留：这页从来只有前端硬编码的常量，不走任何接口。06 文档的信息架构与 §5.7 同步删除。

### 导入页给全部服务商各一条格式示例（同日，用户要求）

粘贴区的 placeholder 一直只有一条 Outlook 的四段样例，而这个框有 16 行高、空着时是
页面上最大的一块留白——用 QQ / 163 / Gmail 的人看完它会以为这里只收 Outlook。
现在 placeholder 跟着「导入格式」下拉走，每种格式一段多行样例（自动识别那一段
故意混了 2 段与 4 段两种写法，因为它就是这么工作的）；下面再加一张固定的示例表，
按 QQ·Foxmail / 163·126 / Gmail / Yahoo / 阿里 / Outlook·Hotmail / 其他域名各给一行。

示例表是后端 `domainProvider` 的用户可见版本，多维护一份的理由是：域名认不认识决定了
2 段写法能不能用，而用户此前唯一的查法是先导一批进去、失败了再回来猜。表下三句提示
对应三条真实的失败路径——填登录密码而不是授权码（报「授权码错误，或未在邮箱设置中
开启 IMAP 服务」）、给 Outlook 写 2 段（微软已停用个人账号的密码登录，导得进来也刷不动）、
企业自建域没走自定义 IMAP。

顺带记一个**没有动**的坑：`outlook.com` 域名写成 2 段时，`ParseLine` 按域名推断出的
`AccountType` 是 `outlook`，于是那一段密码被 `buildImported` 存进 `password_enc` 而不是
