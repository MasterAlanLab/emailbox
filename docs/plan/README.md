...
| [09-ui-revamp.md](09-ui-revamp.md) | `/mail` 工作台改版的现状盘点与方案（已实施，留作决策记录） |
| [10-linear-redesign.md](10-linear-redesign.md) | 视觉语言改成 Linear 风格的规则（已实施，仍是现行规则） |
| [PROGRESS.md](PROGRESS.md) | **实施进度**：各阶段完成状态，以及实现过程中踩到的坑 |

09 与 10 是**某一次改动的记录**，不是持续维护的设计说明：里面提到的组件名可能已经变了
（例如 `GroupTree` 现在是 `GroupList`），以 01–08 与 PROGRESS 为准。
...
P4 之后有两处按需求变化重做的地方，设计文档已同步：
**分组从三级树压平成一层**（2026-08-27，03 §3.1），
**对外取件 API Key**（2026-08-27，只读、一租户一把，范围远小于原 P6，见 07 §5.1）。
...
**分组 → 邮箱账号（批量导入）→ 邮件读取（Graph/IMAP 多通道回退）→ 批量运维（Token 刷新、代理、分组调整）**。
...
| 前端组件库 | Cloudflare **Kumo**（`@cloudflare/kumo`） | React + Base UI + Tailwind v4，与模板技术栈天然匹配；页面实现优先用现成组件，仅虚拟列表 / 邮件正文 / 纵向分栏三处自建 |
