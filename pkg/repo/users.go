...
func mapSQLiteUser(u sqlitedb.User) *model.User {
	return &model.User{ID: u.ID, Username: u.Username, Email: u.Email, PasswordHash: u.PasswordHash, AvatarURL: u.AvatarUrl, Status: model.UserStatus(u.Status), PlatformRole: model.PlatformRole(u.PlatformRole), CreatedAt: u.CreatedAt, UpdatedAt: u.UpdatedAt, DeletedAt: timePtr(u.DeletedAt)}
}
