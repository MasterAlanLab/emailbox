...
- **令牌维护与重新授权**：全部刷新、只刷失败的、或只刷某一个分组；失效的 Outlook 账号可直接从 Token 页重新走 Microsoft OAuth
...
> 默认 Microsoft 应用沿用参考项目。首次在 Token 页重新授权时，若最终跳到
> `http://localhost:8080`，把地址栏里的完整 URL 粘贴回弹窗即可；生产部署自己的回调域名见
